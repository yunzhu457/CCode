...
- [x] 2. 定义配置模型和示例 YAML
  - 影响文件：`internal/config/config.go`、`internal/config/config_test.go`、`configs/config.example.yaml`、`configs/chatgpt-relay.example.yaml`、`configs/anthropic-thinking.example.yaml`、`.gitignore`
  - 依赖任务：任务 1
...
- [x] 8. 增加 Claude extended thinking 处理
  - 影响文件：`internal/provider/anthropic/anthropic.go`、`internal/provider/anthropic/anthropic_test.go`、`configs/anthropic-thinking.example.yaml`
  - 依赖任务：任务 7
...
- [x] 11. 从配置接入 Provider 选择
  - 影响文件：`internal/provider/factory/factory.go`、`internal/provider/factory/factory_test.go`、`cmd/ccode/main.go`
  - 依赖任务：任务 3、任务 6、任务 7
  - 参考位置：`protocol` 字段决定后端协议

- [x] 12. 增加终端安全错误处理和 API Key 脱敏
...
- [ ] 14. 端到端验证
  ...
  - 依赖任务：任务 13
  - 参考位置：`checklist.md`
  - 当前状态：`go test ./...` 和 `go build ./cmd/ccode` 已通过；真实 Provider API 调用会消耗 API 额度，尚未执行。
